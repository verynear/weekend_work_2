...
class Superhero
  ...
  def initialize(input_options_hash)
    ...
    @hitpoints = input_options_hash[:hitpoints].to_i
    @attack = input_options_hash[:attack].to_i
    @alive = true
  ...
  def hit(target)
    if has_special_tool == true
      target.hitpoints = target.hitpoints - (@attack * 3)
    else
      target.hitpoints = target.hitpoints - @attack
    end
